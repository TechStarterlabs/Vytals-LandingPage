...
export default function UserForm() {
  ...
  const fetchRoles = async () => {
    try {
      ...
      setRoles((response.data.roles || []).filter(role => role.name === 'manager'))
    } catch (error) {
  ...
  const fetchUser = async () => {
    try {
      ...
      toast({ title: "Error", description: error.message || "Failed to fetch user details", variant: "destructive" })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">{isEditMode ? "Edit User" : "Add New User"}</h1>
          <p className="text-sm text-gray-500 mt-1">{isEditMode ? "Update user information" : "Create a new admin user"}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="space-y-6">
          ...
          <div>
            ...
            <input type="tel" id="mobile_number" name="mobile_number" value={formData.mobile_number}
              onChange={handleChange}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#338291] focus:border-transparent"
          ...
          <div>
            ...
            <input type="password" id="password" name="password" value={formData.password}
              ...
              required={!isEditMode} />
            {isEditMode && <p className="text-sm text-gray-500 mt-1">Leave blank to keep the current password</p>}
          </div>

          <div className="flex items-center gap-3">
            <input type="checkbox" id="is_active" name="is_active" checked={formData.is_active}
              onChange={handleChange}
              className="w-4 h-4 text-[#338291] border-gray-300 rounded focus:ring-[#338291]" />
            <label htmlFor="is_active" className="text-sm font-medium text-gray-700">Active User</label>
          </div>
